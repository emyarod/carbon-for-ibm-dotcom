...
const template = (props?) =>
  Default({
    [`${prefix}-textarea`]: props,
  });

describe(`${prefix}-textarea`, function () {
  ...
  describe('Rendering', function () {
    it('Should render with various attributes', async function () {
      ...
      expect(
        document.body.querySelector(`${prefix}-textarea` as any)
      ).toMatchSnapshot({
        mode: 'shadow',
      });
    });
    ...
    it('Should reflect value in DOM', async function () {
      ...
      await Promise.resolve();
      expect(
        (document.body.querySelector(`${prefix}-textarea`) as BXTextarea).value
      ).toBe('value-foo');
    });
  });
  ...
  describe('Form validation', function () {
    ...
    beforeEach(async function () {
      ...
      await Promise.resolve();
      elem = document.body.querySelector(`${prefix}-textarea`)!;
    });
