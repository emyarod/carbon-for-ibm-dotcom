...
const template = (props?) =>
  Default({
    [`${prefix}-accordion`]: props,
  });

describe(`${prefix}-accordion`, function () {
  describe('Toggling', function () {
    ...
    beforeEach(async function () {
      ...
      item = document.body.querySelector(`${prefix}-accordion-item`);
    });
    ...
    it('Should fire cds-accordion-item-beingtoggled/cds-accordion-item-toggled events upon opening', async function () {
      ...
      events.on(
        item!,
        `${prefix}-accordion-item-beingtoggled`,
        spyBeforeToggle
      );
      events.on(item!, `${prefix}-accordion-item-toggled`, spyAfterToggle);
      item!.shadowRoot!.querySelector('button')!.click();
      await Promise.resolve();
    ...
    it('Should support preventing modal from being opened upon user gesture', async function () {
      const spyAfterToggle = jasmine.createSpy('after toggle');
      events.on(item!, `${prefix}-accordion-item-beingtoggled`, (event) => {
