...
const definitionTemplate = (props?) =>
  definition({
    [`${prefix}-tooltip-definition`]: props,
  });

const iconTemplate = (props?) =>
  icon({
    [`${prefix}-tooltip-icon`]: props,
  });

describe(`${prefix}-tooltip`, function () {
  describe('Missing menu body', function () {
    ...
    beforeEach(async function () {
      ...
      trigger = document.body.querySelector(`${prefix}-tooltip`);
    });
  ...
  describe('Toggling', function () {
    ...
    beforeEach(async function () {
      ...
      await Promise.resolve();
      trigger = document.body.querySelector(`${prefix}-tooltip`);
      body = document.body.querySelector(`${prefix}-tooltip-body`);
    });
...
describe(`${prefix}-tooltip-definition`, function () {
  describe('Rendering', function () {
    it('Should render with minimum attributes', async function () {
      render(definitionTemplate(), document.body);
      await Promise.resolve();
      expect(
        document.body.querySelector(`${prefix}-tooltip-definition` as any)
...
describe(`${prefix}-tooltip-icon`, function () {
  describe('Rendering', function () {
    it('Should render with minimum attributes', async function () {
      render(iconTemplate(), document.body);
      await Promise.resolve();
      expect(
        document.body.querySelector(`${prefix}-tooltip-icon` as any)
      ).toMatchSnapshot({
        mode: 'shadow',
      });
    });
